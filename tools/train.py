...
def main():
    ...
    if 'dev_check' in cfg.keys():
        if cfg.dev_check.check:
            logger.info(f'Evaluation Check...')
            if cfg.dev_check.input_format == 'NCTHW':
                dummy = torch.rand([10,1,3,10,cfg.dev_check.size, cfg.dev_check.size])
            elif cfg.dev_check.input_format == 'NCHW':
                dummy = torch.rand([10,10,3,cfg.dev_check.size, cfg.dev_check.size])
            with torch.no_grad():
                model.forward_test(dummy)
